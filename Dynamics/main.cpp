				...
				for (int dy = -1; dy <= 1; ++dy) {
					for (int dz = -1; dz <= 1; ++dz) {
						for (Atom particle2 : contains[
							...
							if (abs(particle2.x - particle.x) > bulk_size_x / 2) { //äàëüøå îäíîé êëåòêè
								if (particle2.x > particle.x) {
									particle2.x -= bulk_size_x; //â ñîñåäíþþ êëåòêó ñïðàâà ïåðåéä¸ì
								}
								else {
									particle2.x += bulk_size_x; //â ñîñåäíþþ êëåòêó ñëåâà ïåðåéä¸ì
								}
							}
							if (abs(particle2.y - particle.y) > bulk_size_y / 2) { //äàëüøå îäíîé êëåòêè
								if (particle2.y > particle.y) {
									particle2.y -= bulk_size_y; //â ñîñåäíþþ êëåòêó ñïðàâà ïåðåéä¸ì
								}
								else {
									particle2.y += bulk_size_y; //â ñîñåäíþþ êëåòêó ñëåâà ïåðåéä¸ì
								}
							}
							if (abs(particle2.z - particle.z) > bulk_size_z / 2) { //äàëüøå îäíîé êëåòêè
								if (particle2.z > particle.z) {
									particle2.z -= bulk_size_z; //â ñîñåäíþþ êëåòêó ñïðàâà ïåðåéä¸ì
								}
								else {
									particle2.z += bulk_size_z; //â ñîñåäíþþ êëåòêó ñëåâà ïåðåéä¸ì
								}
							}
...
void elastic_с11(vector <vector<Atom>> &contains, int n_x, int n_y, int n_z, double alpha) {
	for (int i = 0; i < n_x * n_y * n_z; ++i) {
		for (Atom& particle : contains[i]) {
			particle.x = particle.x + alpha * particle.x;
			particle.y = particle.y + alpha * particle.y;
...
void elastic_с12(vector <vector<Atom>> &contains, int n_x, int n_y, int n_z, double alpha) {
	for (int i = 0; i < n_x * n_y * n_z; ++i) {
		for (Atom& particle : contains[i]) {
			particle.x = particle.x + alpha * particle.x;
			particle.y = particle.y - alpha * particle.y;
...
int main(int argc, char* argv[]) {
	...
	//C11 C12
	elastic_с11(curr, n_x, n_y, n_z, alpha);
	...
	elastic_с12(curr, n_x, n_y, n_z, alpha);
